...
# ---------------------------------------------------------------------------
# SIMULATED FLIGHT — full 7-stage journey for John (or any contractor)
...
def _run_100_checks(job: Dict[str, Any], pricing: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # ---- Thermal (20) ----
    eps = float(weather.get("emissivity", 0) or 0)
    add("thermal", "emissivity_calibrated", abs(eps - 0.92) < 1e-6, f"ε={eps}")
    add("thermal", "radiometric_correction", bool(weather.get("ε_corrected")),
        f"corrected={weather.get('ε_corrected')}")
    add("thermal", "temperature_within_op_band",
        20.0 <= float(weather.get("temperature_f", 0) or 0) <= 95.0,
        f"T={weather.get('temperature_f')}°F")
    add("thermal", "wind_within_op_band", float(weather.get("wind_mph", 99) or 99) < 12.0,
        f"wind={weather.get('wind_mph')}mph")
    _precip = weather.get("precip_in")
    add("thermal", "no_precipitation", _precip is not None and float(_precip) == 0.0,
        f"precip={_precip}″")
    add("thermal", "post_sunset_capture", "Post-Sunset" in str(weather.get("sky", "")),
        f"sky={weather.get('sky')}")
...
async def simulation_run(job_id: str = "crown-demo", user=Depends(current_user)):
    """Assembles the full simulation journey for one job. Returns:
      - flight: launch + capture + transfer telemetry summary
      - agents: 4 Claude verdicts (perception, measurement, forensics, pricing) run in parallel
      - checks: 100 deterministic validations
      - twin: 3D digital twin reference
      - pricing: standard pricing breakdown
      - deliverable_route: shortcut to the print-ready packet
    ...
    # Compact briefs (per-agent, kept small to control tokens)
    perception_brief = {
...
# ---------------------------------------------------------------------------
# WEATHER INTELLIGENCE — Open-Meteo (forecast + historical) + RainViewer
# (Doppler radar tiles) + Claude Haiku 4.5 storm-watch / 3-year analyst.
# All upstream services are public & free. No API key required.
# ---------------------------------------------------------------------------
...
def _wx_cache_get(key: str, ttl_s: int) -> Optional[Any]:
    """Tiny in-memory TTL cache for upstream weather responses."""
...
async def weather_forecast(lat: float = DEFAULT_LAT, lon: float = DEFAULT_LON, user=Depends(current_user)):
    ...
    cached = _wx_cache_get(key, ttl_s=600)  # 10-minute TTL
...
async def weather_radar(user=Depends(current_user)):
    ...
    The frontend overlays `host + path` from each radar frame onto Leaflet
    using a TileLayer with a 500ms cycle for animation.
    """
    cached = _wx_cache_get("radar", ttl_s=120)  # 2-minute TTL (RainViewer updates ~5min)
    if cached is not None:
        return cached
    try:
        async with _httpx.AsyncClient(timeout=10.0) as c:
    ...
    out = {
        ...
        "tile_template": "{host}{path}/256/{z}/{x}/{y}/2/1_1.png",  # color 2, snow on, smooth on
        "frames": [
...
async def weather_historical_on_this_day(
    ...
    Pulls Open-Meteo's historical archive for the same MM-DD in each of the
    last N years and emits a compact comparison record.
    """
...
async def weather_storm_watch(body: StormWatchBody, user=Depends(current_user)):
    ...
    Reads the same Open-Meteo forecast we serve to the UI, packs a compact
    signal-rich brief, and asks the model for a structured risk verdict.
    Cheap (one call), cached for 15 min per lat/lon/audience.
    """
...
async def weather_historical_analysis(body: HistoricalAnalysisBody, user=Depends(current_user)):
    """Claude turns the last 3 years of on-this-day weather into a concise brief
    tailored to admin (fleet ops) or contractor (job ops). Cached 1 hour per
    lat/lon/audience.
    """
