...
- `AdminConsensus.jsx` — added `data-testid="admin-consensus-demo-tag"` pill: pink border + pink-tinted bg, 8px uppercase mono. Tooltip on hover: "Verdict generated by Demo · Inject Variance — excluded from compliance reporting".
- Backend `_persist_verdict` already carries the flag through (added in v3.24.0)
- Verified Playwright: 2 DEMO tags rendered on the 2 most recent injected-variance rows; legitimate AUTHENTICATED + the legacy un-flagged REJECTED row both show no tag

Pure visual addition. No data filtering at the API level — compliance teams can still see all verdicts; the tag just makes scanning the feed effortless.
